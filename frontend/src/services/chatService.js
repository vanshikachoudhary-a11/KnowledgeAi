...
export const chatService = {
  list: () => api('/chats'),
  create: (title, documentIds = []) => api('/chats', { method: 'POST', body: JSON.stringify({ title, documentIds }) }),
  messages: (chatId) => api(`/chats/${chatId}`),
  remove: (chatId) => api(`/chats/${chatId}`, { method: 'DELETE' }),
  ...
  streamMessage: async (chatId, message, onToken, signal) => {
    ...
    while (true) { const { value, done } = await reader.read(); if (done) break; buffer += decoder.decode(value, { stream: true }); const events = buffer.split('\n\n'); buffer = events.pop() || ''; for (const event of events) { const data = event.split('\n').find((line) => line.startsWith('data: '))?.slice(6); if (!data) continue; const payload = JSON.parse(data); if (payload.token) onToken(payload.token); if (event.startsWith('event: error')) throw new Error(payload.message || 'The response was interrupted.'); if (event.startsWith('event: done')) result = payload; } }
    return result;
  },
};
